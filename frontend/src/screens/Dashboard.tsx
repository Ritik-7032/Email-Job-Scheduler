...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const refreshCounts = useCallback(async () => {
    try {
      const [schedRes, sentRes] = await Promise.all([
        api.getScheduledEmails(1, 0),
        api.getSentEmails(1, 0),
      ]);
  ...
  useEffect(() => {
    ...
    refreshCounts();
  }, [activeTab, scheduledOffset, sentOffset, fetchScheduled, fetchSent, refreshCounts]);

 
  useEffect(() => {
    const interval = setInterval(() => {
      if (activeTab === 'scheduled') {
        fetchScheduled(scheduledOffset, true);
      } else {
        fetchSent(sentOffset, true);
      }
      refreshCounts();
    }, 3500);
